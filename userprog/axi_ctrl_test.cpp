...
void transfer() {
	...
	auto tx_chans = axidma_get_dma_tx(axidma_dev, &num_tx);
	if (num_tx < 1) {
		std::cerr << "Error: No transmit channels were found." << std::endl;
		std::exit(1);
	}
	auto rx_chans = axidma_get_dma_rx(axidma_dev, &num_rx);
	if (num_rx < 1) {
		std::cerr << "Error: No receive channels were found." << std::endl;
		std::exit(1);
	}

	std::uint32_t *input = (std::uint32_t *)axidma_malloc(axidma_dev, 64);
	std::uint32_t *output = (std::uint32_t *)axidma_malloc(axidma_dev, 64);

	for (std::uint32_t i = 0; i < 16; i++) {
		input[i] = i;
	}

	int rc = axidma_twoway_transfer(axidma_dev, tx_chans[0], input, 64,
																	rx_chans[0], output, 64, true);
	if (rc < 0) {
